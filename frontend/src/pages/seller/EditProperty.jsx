...
const EditProperty = () => {
  ...
  // Fetch initial property details by ID
  useEffect(() => {
  ...
  // Handle newly selected images (with total 10 image limit check)
  const handleNewImageChange = (e) => {
    const files = Array.from(e.target.files);

    if (existingImages.length + newImages.length + files.length > 10) {
      setError("Only 10 images can be uploaded. You cannot exceed 10 images.");
      return;
    }
  ...
  // Remove an image that came from the server
  const removeExistingImage = (url) => {
    setExistingImages(existingImages.filter((img) => img !== url));
  };

  // Remove a newly uploaded image before submission
    ...
    try {
      ...
      setError(
        err.response?.data?.message ||
          "Failed to update that particular property",
      );
    } finally {
      setSubmitting(false);
    }
            ...
            <div className={s.sectionContent}>
              <div>
                ...
                <input
                  ...
                  placeholder="Luxury 3BHK Apartment in Downtown"
                  required
                  className={s.input}
                />
              </div>
          ...
          <div className={s.section}>
            ...
            <div className={s.amenitiesGrid}>
              {commonAmenities.map((amenity) => {
                ...
                return (
                  <label key={amenity} className={s.amenityLabel(isChecked)}>
                    <input
                      type="checkbox"
                      checked={isChecked}
                    ...
                    />
                    <span className={s.amenityText(isChecked)}>{amenity}</span>
                  </label>
                );
              })}
            </div>
